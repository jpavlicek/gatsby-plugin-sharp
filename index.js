...
const {
  boundActionCreators
} = require(`gatsby/dist/redux/actions`); // Promisify the sharp prototype (methods) to promisify the alternative (for
...
const processFile = (file, jobs, cb, reporter) => {
  ...
  jobs.forEach(async job => {
    ...
    if (args.duotone) {
      clonedPipeline = await duotone(args.duotone, args.toFormat || job.file.extension, clonedPipeline);
    }
...
async function notMemoizedbase64({
  file,
  args = {},
  reporter
}) {
  ...
  if (options.duotone) {
    pipeline = await duotone(options.duotone, args.toFormat || file.extension, pipeline);
  }
...
async function fluid({
  ...
  const filteredSizes = fluidSizes.filter(size => size < (fixedDimension === `maxWidth` ? width : height));
...
async function notMemoizedtraceSVG({
  file,
  args,
  fileArgs,
  reporter
  ...
  await new Promise(resolve => pipeline.toFile(tmpFilePath, (err, info) => {
    resolve();
  }));
